...
dav.Client.prototype = {
    ...
    /**
     * Returns the rendered value of a property
     * this methods calls itself for nested properties
     * 
     * @param {String} key
     * @param {String|Object|Array} propValue
     */
    _getPropertyBody: function(key, propValue) {
		...
		if (Array.isArray(propValue)) {
			...
			for(var ii in propValue) {
				// TODO use proper class for this
				if ( propValue[ii].hasOwnProperty('type') && propValue[ii].hasOwnProperty('data') ) {
					body += this._getPropertyBody(propValue[ii].type, propValue[ii].data);
				} else {
					body += this._getPropertyBody(ii, propValue[ii]);
				}
			}
			return '      <' + propName + '>' + body + '</' + propName + '>';
		} else if (typeof propValue === 'object') {
			var body = '';
			// TODO user proper class for this
			if ( propValue.hasOwnProperty('type') && propValue.hasOwnProperty('data') ) {
				return this._getPropertyBody(propValue.type, propValue.data)
			}
			for(var ii in propValue) {
				body += this._getPropertyBody(ii, propValue[ii]);
			}
    ...
    _renderPropSet: function(properties) {
		...
		for(var ii in properties) {
			...
			body += this._getPropertyBody(ii, properties[ii])
		}
		body +='    </d:prop>\n';
		body +='  </d:set>\n';
		return body;
